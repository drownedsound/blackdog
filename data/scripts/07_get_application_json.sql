...
SELECT 
    ...
    -- Audit Trail (Converted to ISO 8601)
    'audit', json_object(
    'created_at', datetime(app.created_at, 'unixepoch'),
    'updated_at', datetime(app.updated_at, 'unixepoch')
    )
    ) AS application_json
...
-- SELECT 
--     json_object(
--     'memberr_reference_no', app.memberr_reference_no,
...
-- WHERE app.memberr_reference_no IN ('APP-2023-SINGLE', 'APP-2023-MULTI');
