...
-- ============================================================================
-- 2. GENERATE DATASET INTO TEMP TABLE
-- ============================================================================
-- We materialize the random structure first. This ensures that the 
-- 'product_fk_id' and 'ref_no' are consistent across all subsequent INSERTs.
...
-- ============================================================================
-- 3. BULK INSERTS (READING TEMP TABLE)
-- ============================================================================
...
INSERT INTO CREDIT_CARD (id, profile_id, credit_limit)
    ...
    FROM temp_dataset WHERE app_type = 'CC';
...
INSERT INTO PERSONAL_LOAN (id, profile_id, loan_amount)
    ...
    FROM temp_dataset WHERE app_type = 'PL';

-- 3.2 Insert Applications
...
-- 3.3 Insert Applicants (Fan-out using Temp Table + Local Name CTEs)
INSERT INTO APPLICANT (
    ...
    first_names(name) AS (
        ...
        -- CORRELATED SUBQUERY: 'WHERE f.n IS NOT NULL' forces re-evaluation per row
        (SELECT name FROM last_names WHERE f.n IS NOT NULL ORDER BY random() LIMIT 1),
    (SELECT name FROM first_names WHERE f.n IS NOT NULL ORDER BY random() LIMIT 1),
    date(abs(random()) % 978307200, 'unixepoch')
    FROM app_fanout f;

    -- 3.4 Insert Contact Numbers
    INSERT INTO CONTACT_NUMBER (applicant_id, type_id, value)
    WITH RECURSIVE
    contact_gen(applicant_id, contact_idx, total_contacts) AS (
    -- Optimization: Only scan applicants inserted in this batch
